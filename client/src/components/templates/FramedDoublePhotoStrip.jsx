...
import framePng from "../../../output-onlinepngtools.png";

export default function FramedDoublePhotoStrip({ photos = [], domRef, onClickSlot }) {
  ...
  const Slot = ({ url, index, accent }) => (
    ...
    >
      {url ? (
        <img src={url} alt={`S${index + 1}`} className="w-full h-full object-cover" />
      ) : (
        <div className="w-full h-full flex items-center justify-center text-secondary-700">Klik untuk pilih foto</div>
      )}
      <img src={framePng} alt="frame" className="absolute inset-0 w-full h-full object-contain pointer-events-none" />
      <div className="absolute bottom-1 right-2 bg-black/50 text-white text-xs px-2 py-0.5 rounded">{index + 1}</div>
    </div>
  );

  return (
    <div ref={domRef} className="flex w-[600px] h-[900px] bg-white p-3 gap-3 border border-primary-200 rounded-3xl">
      <div className="flex flex-col flex-1 rounded-2xl overflow-hidden relative" style={{ background: "linear-gradient(180deg, #f3f4f6 0%, #ffffff 100%)" }}>
      ...
      <div className="flex flex-col flex-1 rounded-2xl overflow-hidden relative" style={{ background: "linear-gradient(180deg, #fef3c7 0%, #fff7ed 100%)" }}>
        <div className="flex flex-col flex-1 p-3 gap-3">
